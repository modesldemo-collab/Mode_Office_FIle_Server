  ...
  return (
    <div className="space-y-5">
          ...
          <thead>
            <tr className="border-b border-[var(--border-main)] bg-[var(--bg-soft)]">
              {["Username", "Email", "Department", "Role", "Status", ""].map((h) => (
                <th key={h} className="text-left px-4 py-3 text-[var(--text-muted)] font-medium">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
              ...
              <tr key={u.id} className="border-b border-[var(--border-main)] hover:bg-[var(--bg-soft)]">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1">
                    <button onClick={() => openEdit(u)} className="text-[var(--text-soft)] hover:text-amber-400 p-1.5 rounded-lg hover:bg-amber-500/10" title="Edit user">
                      <Pencil className="w-4 h-4" />
                    </button>
                    <button onClick={() => openPasswordModal(u)} className="text-[var(--text-soft)] hover:text-cyan-400 p-1.5 rounded-lg hover:bg-cyan-500/10" title="Change password">
                      PW
                    </button>
                    <button onClick={() => handleDeleteUser(u)} className="text-[var(--text-soft)] hover:text-red-400 p-1.5 rounded-lg hover:bg-red-500/10" title="Delete user">
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            <div className="flex-1">
              <label className="block text-xs font-medium text-[var(--text-muted)] mb-1.5 uppercase tracking-wider">Role</label>
            ...
            <div className="flex-1">
              <label className="block text-xs font-medium text-[var(--text-muted)] mb-1.5 uppercase tracking-wider">Active</label>
      ...
      <Modal open={pwdModalOpen} onClose={() => setPwdModalOpen(false)} title={`Set Password: ${pwdUser?.username || "User"}`}>
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-[var(--text-muted)] mb-1.5 uppercase tracking-wider">New Password</label>
            <input
              type="password"
              value={newPassword}
